...
class group_j__servers(JSGroup):
    ...
    def sockexec(self):
        if self._sockexec is None:
            from Jumpscale.servers.sockexec.SockExecFactory import SockExecFactory
            self._sockexec =  SockExecFactory()
    ...
    def taiga(self):
        if self._taiga is None:
            from JumpscaleLibsExtra.servers.taiga.TaigaServers import TaigaServers
            self._taiga =  TaigaServers()
        return self._taiga
    @property
    def bottle_web(self):
        if self._bottle_web is None:
            from threebot_packages.threebot.webinterface.bottle.BottleInterfaceFactory import BottleInterfaceFactory
            self._bottle_web =  BottleInterfaceFactory()
        return self._bottle_web
    @property
    def smtp(self):
        if self._smtp is None:
            from JumpscaleLibs.servers.mail.smtp.SmtpdFactory import SmtpdFactory
            self._smtp =  SmtpdFactory()
        return self._smtp
    @property
    def imap(self):
        if self._imap is None:
            from JumpscaleLibs.servers.mail.imap.imapfactory import ImapServer
            self._imap =  ImapServer()
        return self._imap
...
class group_j__tools(JSGroup):
    ...
    def __init__(self):
        ...
        self._console = None
        self._alerthandler = None
        self._alerthandler_redis = None
        self._licenser = None
        self._path = None
        self._jinja2 = None
    ...
    def alerthandler(self):
        if self._alerthandler is None:
            from Jumpscale.tools.alerthandler.AlertHandler import AlertHandler
            self._alerthandler =  AlertHandler()
        return self._alerthandler
    @property
    def alerthandler_redis(self):
        if self._alerthandler_redis is None:
            from Jumpscale.tools.alerthandler.RedisAlertHandler import AlertHandler
            self._alerthandler_redis =  AlertHandler()
        return self._alerthandler_redis
    @property
    def licenser(self):
    ...
    def tf_gateway(self):
        if self._tf_gateway is None:
            from JumpscaleLibsExtra.tools.tf_gateway.TFGatewayFactory import TFGatewayFactory
            self._tf_gateway =  TFGatewayFactory()
    ...
    def capacity(self):
        ...
        return self._capacity
    @property
    def blog_loader(self):
        if self._blog_loader is None:
            from threebot_packages.threebot.blog.BlogLoader import BlogLoader
            self._blog_loader =  BlogLoader()
        return self._blog_loader
    @property
    def numtools(self):
    ...
    def googleslides(self):
        if self._googleslides is None:
            from JumpscaleLibs.tools.googleslides.GoogleSlides import GoogleSlides
...
class group_j__clients(JSGroup):
    ...
    def carddav(self):
        if self._carddav is None:
            from JumpscaleLibs.clients.carddav.CarddavFactroy import CarddavFactory
            self._carddav =  CarddavFactory()
...
class group_j__data_units(JSGroup):
    ...
    def bytes(self):
        if self._bytes is None:
            from JumpscaleLibs.data.numtools.units.Bytes import Bytes
            self._bytes =  Bytes()
        return self._bytes
    @property
    def sizes(self):
        if self._sizes is None:
            from JumpscaleLibs.data.numtools.units.Sizes import Bytes
            self._sizes =  Bytes()
        return self._sizes
...
class group_j__threebot__package(JSGroup):
    ...
    @property
    def ibiza(self):
        if self._ibiza is None:
            from threebot_packages.examples.ibiza.IbizaTestFactory import IbizaTestFactory
            self._ibiza =  IbizaTestFactory()
        return self._ibiza
    @property
    def helloworld(self):
        if self._helloworld is None:
    ...
    def pastebin(self):
        if self._pastebin is None:
            from threebot_packages.pastebin.PastebinFactory import PastebinFactory
            self._pastebin =  PastebinFactory()
    ...
    def humanitypoll(self):
        if self._humanitypoll is None:
            ...
            self._humanitypoll =  HumanityPollFactory()
        return self._humanitypoll
    @property
    def grid_network(self):
        if self._grid_network is None:
            from threebot_packages.threefold.gridnetwork.GridNetworkFactory import GridNetworkFactory
            self._grid_network =  GridNetworkFactory()
        return self._grid_network
    @property
    def threefold_wikis(self):
        if self._threefold_wikis is None:
            from threebot_packages.threefold.threefold_wikis.ThreefoldWikisFactory import WikisFactory
            self._threefold_wikis =  WikisFactory()
        return self._threefold_wikis
    @property
    def phonebook(self):
    ...
    def freeflownation(self):
        if self._freeflownation is None:
            from threebot_packages.threefold.websites.FreeFlowNationFactory import WebsitesFactory
            self._freeflownation =  WebsitesFactory()
        return self._freeflownation
    @property
    def threefold_io(self):
        if self._threefold_io is None:
            from threebot_packages.threefold.websites.ThreefoldioFactory import WebsitesFactory
            self._threefold_io =  WebsitesFactory()
        return self._threefold_io
    @property
    def tftexplorer(self):
        if self._tftexplorer is None:
            from threebot_packages.threefold.tft_explorer.TFTExplorerFactory import TFTExplorerFactory
            self._tftexplorer =  TFTExplorerFactory()
        return self._tftexplorer
    @property
    def token(self):
        if self._token is None:
            from threebot_packages.threefold.tfgrid_token.TFTokenFactory import TFTokenFactory
            self._token =  TFTokenFactory()
        return self._token
    @property
    def directory(self):
    ...
    def webinterface(self):
        if self._webinterface is None:
            from threebot_packages.threebot.webinterface.WebInterfaceFactory import WebInterfaceFactory
            self._webinterface =  WebInterfaceFactory()
        return self._webinterface
    @property
    def ffmainapp(self):
        if self._ffmainapp is None:
            from threebot_packages.threebot.ffmainapp.FFMainApp import FFMainApp
            self._ffmainapp =  FFMainApp()
        return self._ffmainapp
    @property
    def contacts(self):
        if self._contacts is None:
            from threebot_packages.threebot.contacts.ContactsFactory import ContactsFactory
            self._contacts =  ContactsFactory()
        return self._contacts
    @property
    def interface(self):
        if self._interface is None:
            from threebot_packages.threebot.interface.InterfaceFactory import InterfaceFactory
            self._interface =  InterfaceFactory()
        return self._interface
    @property
    def mail(self):
        if self._mail is None:
            from threebot_packages.threebot.mail.MailFactory import MailFactory
            self._mail =  MailFactory()
        return self._mail
    @property
    def webhooks(self):
        if self._webhooks is None:
            from threebot_packages.threebot.webhooks.WebhooksFactory import WebhooksFactory
            self._webhooks =  WebhooksFactory()
        return self._webhooks
    @property
    def blogs(self):
        if self._blogs is None:
            from threebot_packages.threebot.blog.BlogsFactory import BlogsFactory
            self._blogs =  BlogsFactory()
        return self._blogs
    @property
    def chat(self):
    ...
    def calendar(self):
        if self._calendar is None:
            from threebot_packages.threebot.calendar.CalendarFactory import CalendarFactory
            self._calendar =  CalendarFactory()
        return self._calendar
    @property
    def wikis(self):
        if self._wikis is None:
            from threebot_packages.threebot.wiki.WikisFactory import WikisFactory
            self._wikis =  WikisFactory()
        return self._wikis
    @property
    def appstore2(self):
        if self._appstore2 is None:
            from threebot_packages.threebot.appstore.AppStoreFactory import AppStoreFactory
            self._appstore2 =  AppStoreFactory()
        return self._appstore2
    @property
    def ffbrowser(self):
        if self._ffbrowser is None:
            from threebot_packages.threebot.ffbrowser.FfBrowserFactory import InterfaceFactory
            self._ffbrowser =  InterfaceFactory()
        return self._ffbrowser
    @property
    def gitlab(self):
        if self._gitlab is None:
            from threebot_packages.deploy.gitlab.GitlabFactory import GitlabFactory
            self._gitlab =  GitlabFactory()
        return self._gitlab
    @property
    def odoo(self):
        if self._odoo is None:
            from threebot_packages.deploy.odoo.OdooFactory import OdooFactory
            self._odoo =  OdooFactory()
        return self._odoo
    @property
    def quake(self):
        if self._quake is None:
            from threebot_packages.deploy.quake.QuakeFactory import WordpressFacory
            self._quake =  WordpressFacory()
        return self._quake
    @property
    def wordpress(self):
        if self._wordpress is None:
            from threebot_packages.deploy.wordpress.WordpressFactory import WordpressFacory
            self._wordpress =  WordpressFacory()
        return self._wordpress
    @property
    def ubuntu(self):
        if self._ubuntu is None:
            from threebot_packages.deploy.ubuntu.UbuntuFactory import OdooFactory
            self._ubuntu =  OdooFactory()
        return self._ubuntu
    @property
    def minercraft(self):
        if self._minercraft is None:
            from threebot_packages.deploy.minecraft.MinerCraftFactory import WordpressFacory
            self._minercraft =  WordpressFacory()
        return self._minercraft
    @property
    def taiga(self):
        if self._taiga is None:
            from threebot_packages.deploy.taiga.TaigaFactory import TaigaFactory
            self._taiga =  TaigaFactory()
        return self._taiga
    @property
    def fruum(self):
        if self._fruum is None:
